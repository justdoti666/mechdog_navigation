...
> **⚠ 状态更新（2026-09-25）**：本方案写于"不部署真机"阶段。**现在状态已前进**：
> 感知/安全节点 v2.9.14 已在真机（Pi 5B）跑通 —— 控制路径端到端 **≈34ms**（相机→节点 9ms
> + 处理 17.9ms + 发布 7ms），上位机遥测走新话题 `/safety/terrain_grid`（**0.388 Mbit/s**）。
> 本文档保留作为**电脑端算法验证**的设计记录；文中"非目标：不做真机部署"一条**已作废**。
> 位姿源（odom/IMU）仍未接入 ⇒ 见 §2.3 与"下一步"的 M3。
> 相关：`SOP_MOUNT_CALIB_ON_ROBOT.md`（装机标定）、`CHECKLIST_ON_ROBOT.md`（装机验收）、
> `OFFLINE_TODO.md`（离线待办）。
